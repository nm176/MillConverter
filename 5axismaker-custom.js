...
properties = {
  ...
  setupAtCenterOfCAxis: false, // machine is zeroed at center of C-axis rotation
  safeRetractDistance: (unit == MM ? 12.0 : 0.5), // distance to add to lool length for rewind C-axis tool retract
...
// fixed settings
var pivotDistance = toPreciseUnit(300, MM); // distance to pivot point along B-axis
var cAxisOffset = toPreciseUnit(200, MM); // distance from B-axis CL to C-axis CL
...
function onOpen() {
  ...
  if (true) {
    var aAxis = createAxis({coordinate:1, table:false, axis:[1, 0, 0], range:[-200.00, 200.00]});
    var bAxis = createAxis({coordinate:2, table:false, axis:[0, 0, 1], offset:[cAxisOffset, 0, 0], range:[-180.00, 180.00], cyclic:false});
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0),
        conditional(properties.useG43, gFormat.format(43)),
        zOutput.format(initialPosition.z),
        conditional(properties.useG43, hFormat.format(lengthOffset))
      );
    } else {
      writeBlock(
        gAbsIncModal.format(90),
...
function getOptimizedHeads(_x, _y, _z, _a, _b, _c) {
  ...
  if (true) {
    ...
    if (!properties.setupAtCenterOfCAxis) {
...
function writeRetract() {
  ...
  if (words.length > 0) {
    if (properties.useG28) {
      ...
      // Alternative to G28 retract should be handled here
    }
  }
  zOutput.reset();
}
